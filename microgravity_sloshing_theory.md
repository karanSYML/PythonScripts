...
## 2. The Bond Number — The Governing Dimensionless Parameter

The **Bond number** (Bo) quantifies the relative importance of body acceleration forces to surface tension forces for a liquid in a container:
...
- $a$ ≈ 10⁻⁵ m/s²
- $R_0$ ≈ 0.3 m``
- $\rho$ ≈ 900 kg/m³ (hydrazine-like propellant)
- $\sigma$ ≈ 0.05 N/m

This gives: `Bo = 900 × 10⁻⁵ × 0.3² / 0.05 ≈ 0.016`

Bo << 1 — firmly in the surface-tension-dominated regime. The classical Abramson model is completely inapplicable.

For comparison, during a station-keeping burn with `a ≈ 0.01 m/s²`:
...
### 5.1 Standard Low-g Formula (h > R₀, zero contact angle)
...
$ \omega_1^2  = (a/R_0) · \lambda · \tanh{(\lambda_1 \cdot h/R_0)} \cdot [1 + (\lambda_1^2 · N_{BO}) / (ω̄₁²)]^{1/2} $


In the simplified form used for engineering calculations (Jang et al. 2014, Eq. 38 — 90° contact angle limit):

```
ω₁² = (a · λ₁ / R₀) · tanh(λ₁ · h/R₀) + (λ₁² · σ / (ρ · R₀³)) · [λ₁² - 1] · tanh(λ₁ · h/R₀)
```

where:
- `λ₁ = 1.8412` (first zero of the derivative of the first-order Bessel function J₁')
- `σ` is the liquid surface tension [N/m]
- `N_BO = Bo = ρ · a · R₀² / σ` is the Bond number
...
```
ω²_grav = (a · λ₁ / R₀) · tanh(λ₁ · h/R₀)
```

and the surface tension (capillary) term is:
```
ω²_cap = (σ · λ₁² · (λ₁² - 1)) / (ρ · R₀³) · tanh(λ₁ · h/R₀)
```
...
```
ω₁|_(a=0) = sqrt[(σ · λ₁² · (λ₁² - 1)) / (ρ · R₀³) · tanh(λ₁ · h/R₀)]
```
...
```
m₁ / m_liq = (2 / λ₁²) · [1 / (1 + N_BO / (λ₁² - 1))] · tanh(λ₁ · h/R₀) · I_n
```
...
```
m₁/m_liq ≈ (2 / (λ₁² · h/R₀)) · tanh(λ₁ · h/R₀) · f(Bo)
```
...
```
ζ₁ = 0.83 · N_GA^(-1/2),          for Bo ≤ 10
ζ₁ = 0.83 · N_GA^(-1/2) + 0.096 · N_BO^(-3/5) · N_GA^(-1/2),    otherwise
```
...
and `ν_k` is the kinematic viscosity of the propellant [m²/s].
...
## 8. Free Surface Stability and Critical Acceleration
...
## 9. Correction for Small Fill Fractions (h < R₀)
...
## 10. Correction for Elliptical Dome Tanks
...
The Jang et al. (2014) Mechanical Slosh Toolbox (MST) implements exactly this 2-D lookup for the low-g regime, with Bond number as the second dimension.
